...
import java.util.List;
    ...
    public class BranchController {
        ...
        @GetMapping("/add")
        public ModelAndView newBranch (Model model){
            model.addAttribute("branch", new Branch());
            return new ModelAndView("branch/add",model.asMap());
        }

        @PostMapping("/add")
        public ModelAndView addBranch (@Valid @ModelAttribute Branch branch, BindingResult result, Model model){
            if(result.hasErrors()){
                model.addAttribute("branch", new Branch());
        ...
        public ModelAndView showUpdateBranch (@PathVariable("id") Integer branchId, Model model){
        ...
        public ModelAndView updateBranch (@Valid @ModelAttribute Branch branch, BindingResult result, @PathVariable("id") Integer branchId, Model model){
            if(result.hasErrors()){
                model.addAttribute("error", "All fields are required. Please complete all fields.");
                return new ModelAndView("branch/update",model.asMap());
            } else{
        ...
        public ModelAndView deleteBranch (@PathVariable("id") Integer branchId, Model model){
            boolean doIt = branchServices.deleteBranch(branchId);
            return new ModelAndView("redirect:/branch/index",model.asMap());
        }

        @GetMapping("/getOne")
        public ModelAndView getOneBranch (@RequestParam(value = "id", required = true) String branchId, Model model) {
            try {
                int id = Integer.parseInt(branchId);
                BranchDTO branchDTO = branchServices.getOneBranchDTO(id);
                if (branchDTO != null) {
                    model.addAttribute("branchs", List.of(branchDTO));
                } else {
        ...
        @GetMapping("/getAll")
        public ModelAndView getAllBranch (Model model){
            List<BranchDTO> branchList = branchServices.getAllBranch();
